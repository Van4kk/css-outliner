<template>
    <svg v-if="iconExists" :width="width" :height="height" v-html="iconName" :class="className" :fill="color" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg"></svg>
    <svg v-else class="text-red-500" xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 64 64" xml:space="preserve">
<defs id="defs2"/>
...
export default {
    ...
    setup(props) {
        const iconExists = ref(props.iconName in icons);

        const iconName = computed(() => {
            return iconExists.value ? icons[props.iconName] : null;
        });
